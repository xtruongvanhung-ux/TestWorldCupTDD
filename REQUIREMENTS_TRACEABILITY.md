...
between the functional requirements defined in the SRS and the
implemented test cases.

The purpose of the RTM is to verify that the functional requirements
are mapped to corresponding automated test cases and to identify any
requirement that is not fully covered by the current test suite.

The current test suite contains 257 automated test cases, including JUnit 5 unit tests and Selenium Web UI tests.
...
substitutes, and staff information.
...
Boundary Value Analysis is also applied to the numerical match rules.
...
ranking criteria are evaluated in sequence.
...
The SRS also specifies that the web application shall handle invalid
numeric input. The current Selenium test suite does not contain a
specific test case for invalid non-numeric input.

Therefore, FR-09 is only partially covered by the current test suite.

**Coverage status:** Partially Covered

**Coverage gap:** Invalid numeric input is not currently covered by a
dedicated Selenium test case.
...
The current automated test suite contains 256 test cases.
...
| FR-09 — Web Application | Partially Covered |
...
The only identified coverage gap is the invalid non-numeric input
condition specified in FR-09.
...
FR-01 through FR-08 are covered by the existing JUnit test suites.

FR-09 is covered by the Selenium Web UI tests, including valid
...
257 test cases passed suc# Requirements Traceability Matrix
...
The additional TC_WEB_006 test verifies that invalid non-numeric input
is handled correctly and that the application displays:
